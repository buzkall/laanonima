...
it('accepts a valid ISBN-13 however it is typed', function(string $input) {
...
it('rejects a broken check digit', function() {
    expect(Isbn::isValid('9788433920424'))->toBeFalse()
        ->and(Isbn::toIsbn13('9788433920424'))->toBeNull();
});

it('rejects anything that is not the right length', function(?string $input) {
    expect(Isbn::isValid($input))->toBeFalse();
})->with(['123', '', null, '97884339204233', 'not-an-isbn']);

it('converts an ISBN-10 up to its 13-digit form', function() {
    expect(Isbn::toIsbn13('8433920421'))->toBe('9788433920423');
});

it('converts a 978-prefixed ISBN-13 back down to 10 digits', function() {
    expect(Isbn::toIsbn10('9788433920423'))->toBe('8433920421');
});

it('has no ISBN-10 form for a 979 prefix', function() {
...
it('handles the X check digit of an ISBN-10', function() {
