...
describe('enriching what was filed', function(): void {
    ...
    it('writes a breadcrumb before every step, so a killed process says where it stopped', function(): void {
        Log::spy();
        ...
        /* Each of these is written before the work it names, which is the whole
           point: the enrichment runs in a terminating callback where a timeout
           or an exhausted heap is a fatal no catch sees, so the last line in
           the file is the only thing that says what was being done. */
        foreach (['[enrich] started', '[enrich] metadata lookup', '[enrich] cover download', '[enrich] finished'] as $breadcrumb) {
            Log::shouldHaveReceived('log')->withArgs(
                fn(string $level, string $message): bool => $message === $breadcrumb,
            )->once();
        }
    });

    it('names the source that was being asked when a lookup stops', function(): void {
        Log::spy();
        ...
        /* A provider swallows its own failures by contract, so a source that is
           down and a source that answered in a millisecond leave the same trace
           without this: the name of the one being asked, before it is asked. */
        Log::shouldHaveReceived('log')->withArgs(
            fn(string $level, string $message): bool => $message === '[metadata] asking OpenLibraryProvider',
        )->once();
    });

    it('keeps the trail out of the log when it is switched off', function(): void {
        config(['books.metadata.trace' => false]);
