...
it('searches by author, not just by title', function() {
...
it('searches by ISBN, shown under the title', function() {
...
it('searches by publisher, shown under the author', function() {
...
it('rejects an ISBN whose check digit does not add up', function() {
    livewire(CreateBook::class)
...
it('explains a rejected ISBN instead of printing a translation key', function(string $locale) {
...
it('requires an ISBN and a title', function() {
    livewire(CreateBook::class)
...
it('refuses to shelve the same ISBN twice', function() {
...
it('shows the price in euros while storing it in cents', function() {
...
describe('the ISBN lookup', function() {
    it('fills the form from the metadata providers', function() {
        Http::fake([
    ...
    it('creates the publisher it found, so it is a real relationship', function() {
        Http::fake([
            'openlibrary.org/api/books*' => Http::response(apiFixture('book-metadata/open-library-hit')),
            'covers.openlibrary.org/*'   => Http::response('', 404),
        ]);
    ...
    it('asks the bookseller to type it in when nothing is found', function() {
    ...
    it('does not blank out what the bookseller already typed', function() {
        Http::fake([
    ...
    it('saves where the metadata came from, and when', function() {
        Http::fake([
            'openlibrary.org/api/books*' => Http::response(apiFixture('book-metadata/open-library-hit')),
            'covers.openlibrary.org/*'   => Http::response('', 404),
        ]);
    ...
    it('says so when the ISBN itself is wrong, without calling anyone', function() {
        Http::fake();
