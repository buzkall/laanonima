...
<head>
    ...
    <title>
        {{ $book->title }}{{ $book->authors_line ? ', de ' . $book->authors_line : '' }} — {{ config('app.name') }}
    </title>
    <meta
        name="description"
...
>
    ...
    <div class="wide:grid-cols-[0.92fr_1.08fr] wide:items-stretch grid grid-cols-1">
        ...
        <div class="wide:h-full wide:self-stretch wide:bg-transparent wide:p-0 relative bg-[var(--cover)] px-[22px] pt-10 pb-12">
            <div data-band class="wide:block absolute inset-x-0 top-0 hidden bg-[var(--cover)]"></div>

            <div
                data-cover-sticky
                class="wide:pointer-events-none wide:fixed wide:top-[var(--top-bar)] wide:bottom-0 wide:left-0 wide:z-30 wide:box-border wide:flex wide:w-[46%] wide:items-start wide:justify-start wide:pt-[clamp(24px,3.5vw,56px)] wide:pb-[clamp(16px,2vw,36px)] wide:pl-[clamp(16px,2vw,36px)] static"
            >
                @if ($cover)
                    <img
                        src="{{ $cover }}"
                        ...
                        data-cover
                        class="wide:m-0 wide:max-h-full wide:w-auto wide:max-w-full wide:origin-top-left wide:will-change-transform mx-auto block h-auto w-full max-w-[420px] shadow-[0_2px_8px_rgba(33,21,17,0.18),0_26px_60px_rgba(33,21,17,0.34)]"
                    />
                @else
                    <div
                        data-cover
                        class="wide:m-0 wide:h-full wide:max-h-[600px] wide:w-auto wide:max-w-full wide:min-w-[280px] mx-auto flex aspect-[2/3] w-full max-w-[420px] items-end border border-[var(--rule)] p-6 text-[var(--on-cover)] shadow-[0_2px_8px_rgba(33,21,17,0.18),0_26px_60px_rgba(33,21,17,0.34)]"
                    >
        ...
        <div class="min-w-0">
            ...
            >
                <h1 class="font-display m-0 text-[clamp(52px,6.4vw,104px)]/[0.92] font-normal tracking-[-0.01em] text-balance">
                    {{ $book->title }}
                </h1>
                ...
                <div class="mt-11 flex flex-wrap items-center gap-x-7 gap-y-4">
                    ...
                    <a
                        href="{{ $cta['href'] }}"
                        data-buy
                        class="bg-[var(--on-cover)] px-6 py-3 text-[16px] font-semibold tracking-[0.06em] text-[var(--cover)] uppercase transition-opacity duration-150 hover:opacity-85"
                    >
                        {{ $cta['label'] }}
                    </a>
            ...
            <div class="bg-paper text-ink px-[clamp(22px,4vw,56px)] pt-[clamp(48px,5vw,76px)] pr-[clamp(22px,4vw,64px)] pb-[clamp(56px,6vw,84px)]">
                @if ($book->synopsis)
                    <section class="max-w-[560px]">
                        <h2 class="m-0 mb-[22px] text-[14px] font-bold tracking-[0.26em] text-[var(--accent)] uppercase">
                            {{ __('books.public.synopsis_kicker') }}
                        </h2>
                        <div class="flex flex-col gap-[22px]">
                            ...
                            @if ($book->back_cover_text)
                                <p class="m-0 border-l-2 border-[var(--accent)] pl-5 italic">
                    ...
                    'mt-[clamp(56px,6vw,88px)] border-t border-ink pt-11' => filled($book->synopsis),
                            ...
                            <div class="mb-[34px] grid grid-cols-[repeat(auto-fill,minmax(min(100%,150px),1fr))] gap-3">
                                @foreach ($gallery as $image)
                                    <img
                                        ...
                                        loading="lazy"
                                        class="border-ink block h-auto w-full border"
                                    />
                                @endforeach
                            </div>
                        @endif
                        ...
                        <dl class="m-0 flex flex-col">
                            @foreach ($facts as $label => $value)
                                <div class="border-ink flex items-baseline justify-between gap-4 border-b border-dotted py-[13px]">
                                    <dt class="text-[14px] font-semibold tracking-[0.14em] uppercase">{{ $label }}</dt>
                                    <dd class="m-0 text-right text-[19px] italic">
                                        @isset($factLinks[$label])
                                            <a
                                                href="{{ $factLinks[$label] }}"
                                                class="text-[var(--accent)] transition-opacity duration-150 hover:opacity-65"
                                            >{{ $value }}</a>
                                        @else
                                            {{ $value }}
                                        @endisset
    ...
    @if ($contributors->isNotEmpty())
        <section class="border-ink bg-paper text-ink wide:pl-[46vw] border-t px-[clamp(22px,5vw,80px)] pt-[clamp(52px,5vw,88px)] pb-[clamp(60px,6vw,96px)]">
            <h2 class="m-0 mb-[34px] text-[14px] font-bold tracking-[0.26em] text-[var(--accent)] uppercase">
                {{ __('books.public.authors_kicker') }}
            </h2>
            ...
            <div class="grid max-w-[860px] grid-cols-[repeat(auto-fit,minmax(240px,1fr))] items-start gap-x-12 gap-y-10">
                <ul class="m-0 flex list-none flex-col gap-3 p-0">
                    @foreach ($contributors as $person)
                        <li class="border-ink border-b border-dotted pb-[10px]">
                            <div class="flex items-baseline justify-between gap-4">
                                <a
                                    href="{{ $person['href'] }}"
                                    class="font-display text-[26px] text-[var(--accent)] transition-opacity duration-150 hover:opacity-65"
                                >{{ $person['name'] }}</a>
                                <span class="text-[16px] whitespace-nowrap italic">{{ $person['role']->getLabel() }}</span>
                            </div>
                ...
                @if ($alsoByAuthors->isNotEmpty())
                    <div class="border-ink border-t pt-[18px]">
                        <h3 class="m-0 mb-3 text-[13px] font-bold tracking-[0.2em] uppercase">
                            {{ __('books.public.also_by_them') }}
                        </h3>
                        <ul class="m-0 flex list-none flex-col gap-[10px] p-0">
                            @foreach ($alsoByAuthors as $other)
                                <li class="border-ink flex items-baseline justify-between gap-4 border-b border-dotted pb-[10px]">
                                    <a
                                        href="{{ route('books.show', $other) }}"
                                        class="font-display text-[22px] text-[var(--accent)] transition-opacity duration-150 hover:opacity-65"
                                    >{{ $other->title }}</a>
                                    @if ($other->published_year)
                                        <span class="text-[16px] whitespace-nowrap italic">{{ $other->published_year }}</span>
                                    @endif
                                </li>
                            @endforeach
                        </ul>
    ...
    @if ($book->publisher && ($book->publisher->description || $fromPublisher->isNotEmpty()))
        <section class="wide:pr-[clamp(22px,5vw,80px)] wide:pl-[46vw] bg-[var(--cover)] px-[max(clamp(22px,5vw,80px),calc(50vw-320px))] pt-[clamp(52px,5vw,88px)] pb-[clamp(60px,6vw,96px)] text-[var(--on-cover)]">
            <div class="mx-auto max-w-[640px] text-center">
                <h2 class="m-0 mb-[18px] text-[14px] font-bold tracking-[0.26em] uppercase">
                    {{ __('books.public.publisher_kicker', ['publisher' => $book->publisher->name]) }}
                </h2>

                @if ($book->publisher->logoUrl('thumb'))
                    <img
                        src="{{ $book->publisher->logoUrl('thumb') }}"
                        alt="{{ $book->publisher->name }}"
                        class="mx-auto mb-7 block h-[72px] w-auto"
                    />
                @endif
                    ...
                    <ul class="m-0 flex list-none flex-col p-0">
                        @foreach ($fromPublisher as $other)
                            ...
                            'border-b' => $loop->last,
                                ...
                                <a
                                    href="{{ route('books.show', $other) }}"
                                    class="font-display text-[26px] transition-opacity duration-150 hover:opacity-65"
                                >{{ $other->title }}</a>
                ...
                <a
                    ...
                    class="mt-9 inline-block border-b-2 border-current pb-[3px] text-[15px] font-semibold tracking-[0.12em] uppercase transition-opacity duration-150 hover:opacity-65"
                >
    ...
    <footer
        id="comprar"
        class="bg-paper text-ink px-[max(clamp(22px,5vw,80px),calc(50vw-320px))] pt-[clamp(64px,7vw,104px)] pb-[clamp(72px,8vw,112px)] text-center"
    >
        <p class="font-display mx-auto my-0 max-w-[620px] text-[clamp(34px,6vw,58px)]/[1.05] text-balance">
            {{ __("books.public.{$stock}.heading") }}
        </p>
        ...
        <p class="mt-16 mb-0 text-[14px] tracking-[0.2em] uppercase opacity-70">
    ...
    <div class="wide:hidden h-[84px]" aria-hidden="true"></div>

    <div
        data-buy-bar
        class="wide:hidden fixed inset-x-0 bottom-0 z-40 flex translate-y-full items-center gap-4 border-t border-[var(--rule)] bg-[var(--cover)] px-[22px] py-3 text-[var(--on-cover)] transition-transform duration-200"
    >
        @if ($book->priceInEuros() !== null)
        ...
        <a
            href="{{ $cta['href'] }}"
            class="ml-auto bg-[var(--on-cover)] px-5 py-3 text-[15px] font-semibold tracking-[0.06em] text-[var(--cover)] uppercase transition-opacity duration-150 hover:opacity-85"
        >
            {{ $cta['label'] }}
        </a>
    </div>
    ...
    <script>
     ...
     |
     | On a wide screen, the cover starts centered in its column, against the
     | hero color, and slides left until it is half cut off by the edge of the
     | window, where it stays floating over the cream. The colored band behind
     ...
     */
        (function () {
            ...
            const paint = () => {
                ...
                cover.style.transform =
                    'translateX(' + (-eased * (atRest.left + atRest.width * 0.5)).toFixed(1) + 'px)';
            };
