...
`ShopScraper` lives in `App\Support\Shop`, not `App\Support\Cupida`, because the real catalogue must not depend on a namespace that gets deleted when La Cupida's stand-in pool does.
